<template>
    <section class="flex flex-col rounded-md bg-gradient-to-br from-stone-50 to-stone-200 p-6 shadow-inner inset-shadow-2xs inset-shadow-zinc-200">
        <h2 class="mb-4 text-xl font-semibold text-stone-800">{{ t("shorts") }}</h2>
        <div v-if="loading" class="flex justify-center items-center h-60">
            <span class="text-stone-500">{{ t('loading') }}...</span>
        </div>
        <div v-else-if="error" class="flex justify-center items-center h-60">
            <span class="text-red-500">{{ t('error.fetch') }}</span>
        </div>
        <div v-else class="-mr-2 h-60 flex-1 space-y-3 overflow-y-auto pr-2 md:h-auto">
            <div v-for="short in latestShorts" :key="short.videoId"
                ...
                <div class="min-w-0">
                    ...
                    <p class="text-xs text-stone-500">{{ short.views }} {{ t('views') }}</p>
                </div>
            </div>
        </div>
    </section>
